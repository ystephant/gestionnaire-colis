...
// ═══════════════════════════════════════════════════════
//  GEAR SVG — redesigned: clean knurled dial look
//  20 deep rectangular teeth, flat outer ring, machined look
// ═══════════════════════════════════════════════════════
const GEAR_TEETH  = 20;
const GEAR_RI     = 33;   // inner radius of teeth
const GEAR_RO     = 46;   // outer radius of teeth
const GEAR_TW     = 0.30; // tooth width ratio

function buildGearPath(teeth = GEAR_TEETH, ri = GEAR_RI, ro = GEAR_RO, tw = GEAR_TW) {
  const step = (2 * Math.PI) / teeth;
  let d = "";
  for (let i = 0; i < teeth; i++) {
    const b = i * step;
    const gap = step * (0.5 - tw);
    const pts = [
      [b - step * tw + gap * 0.15, ri],
      [b - step * tw,              ro],
      [b + step * tw,              ro],
      [b + step * tw - gap * 0.15, ri],
    ];
...
// Tick marks on the inner disk face (like a tuning dial)
const TICK_COUNT = 40;
...
function GearSVG({ rotation, lit }) {
  return (
    <svg width="100%" height="100%" viewBox="0 0 100 100" style={{ display:"block", overflow:"visible" }}>
      ...
      <g transform={`rotate(${rotation} 50 50)`} filter="url(#gShadow)">
        {/* ── Outer gear body ── */}
        <path d={GEAR_PATH} fill="url(#gBody)" />
        {/* Bevel sheen on teeth */}
        <path d={GEAR_PATH} fill="url(#gBevel)" opacity="0.6"/>
        {/* Outer ring groove */}
        <circle cx="50" cy="50" r={GEAR_RI - 1} fill="none" stroke="rgba(0,0,0,0.35)" strokeWidth="1.5"/>
        <circle cx="50" cy="50" r={GEAR_RI}     fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="0.6"/>

        {/* ── Inner knurled disk ── */}
        <circle cx="50" cy="50" r={GEAR_RI - 1.5} fill="url(#gDisk)"/>
        ...
        {Array.from({length: TICK_COUNT}).map((_, i) => {
          ...
          const major = i % 5 === 0;
          const r1 = GEAR_RI - 3;
          const r2 = major ? GEAR_RI - 8 : GEAR_RI - 5.5;
          return (
            <line key={i}
        ...
        {[0, 72, 144, 216, 288].map(deg => {
          ...
          return (
            <line key={deg}
              ...
              x2={50 + Math.cos(a) * 22} y2={50 + Math.sin(a) * 22}
              stroke="rgba(255,255,255,0.14)" strokeWidth="3" strokeLinecap="round"/>
          );
        })}

        {/* ── Hub ── */}
        <circle cx="50" cy="50" r="9"   fill="#1a1a1a" stroke="rgba(255,255,255,0.12)" strokeWidth="0.8"/>
        <circle cx="50" cy="50" r="5.5" fill="#2d2d2d"/>
        <circle cx="50" cy="50" r="2.5" fill="#888"/>
        {/* Hub highlight */}
        <circle cx="47" cy="47" r="1.2" fill="rgba(255,255,255,0.4)"/>

        {/* ── Active glow ── */}
        {lit && <circle cx="50" cy="50" r={GEAR_RI - 1.5} fill="url(#gLit)"/>}
      </g>
    </svg>
  );
}
...
// ═══════════════════════════════════════════════════════
//  GEAR — interactive drag widget
//  Fixed: 1px drag = fractional degree, accumulate until
//  threshold, emit integer points. Much more predictable.
// ═══════════════════════════════════════════════════════
const PX_PER_POINT = 28;  // pixels of drag per score point
const DEG_PER_PT   = 360 / GEAR_TEETH;  // degrees to rotate per point

function Gear({ onChange, size = 110 }) {
  const [rotation, setRotation] = useState(0);
  const [lit, setLit]           = useState(false);
  const drag    = useRef(null);   // { lastY, acc }
  ...
  // Called with raw dy (positive = down). Up = positive score.
  const processDy = useCallback((dy) => {
    if (!drag.current) return;
    // up = negative dy = positive score
    drag.current.acc -= dy;
    const pts = Math.trunc(drag.current.acc / PX_PER_POINT);
    if (pts !== 0) {
      drag.current.acc -= pts * PX_PER_POINT;
      setRotation(r => r - pts * DEG_PER_PT);
      cbRef.current(pts);
      clearTimeout(litRef.current);
      setLit(true);
      litRef.current = setTimeout(() => setLit(false), 120);
    }
  }, []);
  ...
  // Scroll wheel: each notch = 1 point
  useEffect(() => {
  ...
  return (
    <div
      ...
      style={{
        ...
        flexShrink: 0,
        filter: lit ? "drop-shadow(0 0 8px rgba(255,190,50,0.7))" : "none",
...
// ═══════════════════════════════════════════════════════
//  HOLD ARROW — press and hold to repeat
// ═══════════════════════════════════════════════════════
function HoldArrow({ direction, onTick, color, bgColor }) {
  ...
  return (
    <button
      ...
      onMouseDownCapture={e => e.currentTarget.style.transform = "scale(0.87)"}
...
export default function ScoreTracker() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:D.bg, color:D.txt, fontFamily:"'Sora',sans-serif", transition:"background .3s" }}>
      ...
      <main style={{ maxWidth:900,margin:"0 auto",padding:"18px 14px 150px",overflow:"visible" }}>

        {/* ══════════════════════ SCORES TAB ══════════════════════ */}
        {tab === "scores" && (<div style={{overflow:"visible"}}>
          {players.length===0 && (
          ...
          {/* overflow:visible so the gear can protrude above/below each band */}
...
// ═══════════════════════════════════════════════════════
//  PLAYER ROW
//  Layout: slim band (≈64px) with gear overflowing above+below.
//  Gear is 2× the band height = ~128px, centered vertically.
//  Arrows sit at the very top and bottom of the gear column.
// ═══════════════════════════════════════════════════════
const ROW_H    = 64;   // slim band height in px
const GEAR_SZ  = 128;  // gear diameter = 2× the band
...
function PlayerRow({ player, rank, medal, isLeader, isSel, flashKey, delta, dark, onAdjust, onRemove, onSelect, onColorChange }) {
  ...
  // Overflow = (GEAR_SZ - ROW_H) / 2 each side
  const overflow = (GEAR_SZ - ROW_H) / 2; // 32px

  return (
    // Outer wrapper: clips nothing, just provides the stacking context
    <div style={{
      position: "relative",
      // Extra vertical padding so the gear doesn't get clipped
      paddingTop: overflow,
      paddingBottom: overflow,
      // Collapse the margin so rows look normally spaced
      marginTop: rank === 0 ? 0 : -overflow * 1.2,
      zIndex: isSel ? 10 : 1,
    }}>
      {/* ── Slim coloured band ── */}
      <div
        ...
        style={{
          ...
          background: col,
          cursor: "pointer",
          overflow: "visible",   // gear can stick out
          boxShadow: isSel
            ? `inset 0 0 0 9999px rgba(0,0,0,0.12), 0 0 0 3px ${col}, 0 0 0 5.5px rgba(255,255,255,0.22), 0 8px 32px ${col}66`
            : `inset 0 0 0 9999px rgba(0,0,0,0.48), 0 2px 10px ${col}30`,
      ...
      >
        {/* Rank badge */}
        <div style={{ minWidth:26,height:26,borderRadius:8,background:"rgba(255,255,255,0.14)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:rank<3?15:10,fontWeight:800,color:"#fff",flexShrink:0 }}>
          {medal}
        </div>
        ...
        <div style={{ position:"relative",flexShrink:0 }} data-pal>
          <button
            ...
            style={{ width:38,height:38,borderRadius:"50%",background:"rgba(255,255,255,0.18)",backdropFilter:"blur(4px)",border:"2px solid rgba(255,255,255,0.35)",color:"#fff",fontWeight:800,fontSize:isLeader?17:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}
          ...
          {showPalette && (
            <div className="fade-in" data-pal
              style={{ position:"absolute",top:46,left:0,zIndex:30,background:dark?"#1c1f2e":"#fff",border:"1px solid rgba(255,255,255,0.1)",borderRadius:14,padding:10,display:"flex",flexWrap:"wrap",gap:7,width:152,boxShadow:"0 12px 36px rgba(0,0,0,.5)" }}
        ...
        <div style={{ flex:1,minWidth:0 }}>
          ...
          <div style={{ fontSize:9,color:"rgba(255,255,255,0.45)",marginTop:1 }}>
            {isLeader ? "👑 En tête" : `#${rank+1} · ${player.color.name}`}
          </div>
        </div>

        {/* Score */}
        <div style={{ position:"relative",textAlign:"center",minWidth:70,flexShrink:0 }}>
          <div key={flashKey} className="mono score-pop"
            style={{ fontSize:42,fontWeight:700,color:"#fff",letterSpacing:-2,lineHeight:1,textShadow:"0 2px 12px rgba(0,0,0,0.3)" }}>
            {player.score > 0 ? "+" : ""}{player.score}
          </div>
          {delta !== undefined && (
            <div className="delta-fly mono" style={{ position:"absolute",top:-4,right:-8,zIndex:5,fontSize:17,fontWeight:700,color:delta>0?"#86efac":"#fca5a5" }}>
              {delta > 0 ? "+" : ""}{delta}
            </div>
          )}
        </div>

        {/* ── Gear column: overflows the band ── */}
        <div
          data-gear
          style={{
            position: "absolute",
            right: 52,        // leave room for ✕ button
            top: "50%",
            ...
            alignItems: "center",
            gap: 2,
            // total height = GEAR_SZ + 2 arrows (36px each) + 2 gaps (2px each)
            height: GEAR_SZ + 2 * 36 + 2 * 2,
            zIndex: 5,
          }}
        >
          {/* ▲ up arrow */}
        ...
        {/* Remove button — needs right margin to not overlap gear */}
        <div style={{ width: 58, flexShrink: 0 }} /> {/* spacer for gear column */}
        <button data-rm onClick={e => { e.stopPropagation(); onRemove(); }}
          style={{ width:26,height:26,borderRadius:7,border:"none",flexShrink:0,background:"rgba(255,255,255,0.12)",color:"rgba(255,255,255,0.5)",cursor:"pointer",fontSize:13,transition:"background .13s,color .13s",zIndex:6,position:"relative" }}
          onMouseEnter={e => { e.currentTarget.style.background="rgba(239,68,68,0.5)"; e.currentTarget.style.color="#fff"; }}
          onMouseLeave={e => { e.currentTarget.style.background="rgba(255,255,255,0.12)"; e.currentTarget.style.color="rgba(255,255,255,0.5)"; }}>
          ✕
        </button>
      </div>
    </div>
